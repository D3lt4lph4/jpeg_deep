...
class TrainingConfiguration(object):
    ...
    def __init__(self):
        ...
        # Network variables
        self._weights = "/dlocal/home/2017018/bdegue01/weights/jpeg_deep/classification_dct/lcrfa_y/classification_dct_jpeg-deep_46lWNDTTTHMqAvNj88wqA2HPofUMhHva/checkpoints/epoch-71_loss-0.9617_val_loss-1.6794.h5"
        self._network = SSD300_resnet(
            backbone="lcrfa_y", dct=True, image_shape=(38, 38))
    ...
    def prepare_for_inference(self):
        K.clear_session()
        self._network = SSD300_resnet(
            backbone="lcrfa_y", dct=True, image_shape=(38, 38), mode="inference")

    def prepare_evaluator(self):
        self._evaluator = PascalEvaluator(challenge="VOC2012")

    def prepare_testing_generator(self):
        self._test_generator = VOCGenerator(batch_size=self.batch_size, shuffle=False, label_encoder=self.input_encoder, dct=True, train_mode=False, only_y=True,
                                            transforms=self.test_transformations, images_path=self.test_sets)
        self._test_generator.prepare_dataset()

    def prepare_training_generators(self):
        self._train_generator = VOCGenerator(batch_size=self.batch_size, shuffle=True, label_encoder=self.input_encoder, dct=True, only_y=True,
                                             transforms=self.train_tranformations, images_path=self.train_sets)
        self._train_generator.prepare_dataset()
        self._validation_generator = VOCGenerator(batch_size=self.batch_size, shuffle=True, label_encoder=self.input_encoder, dct=True, only_y=True,

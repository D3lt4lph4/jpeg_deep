...
def vggd_dct_resize(classes=1000):
    ...
    norm_cbcr = BatchNormalization(
        name="b_norm_128", input_shape=input_shape_cbcr)(input_cbcr)
    norm_cbcr = ResizeFeatures((14, 14))(norm_cbcr)

    # Block 1
    x = BatchNormalization(
        name="b_norm_64", input_shape=input_shape_y)(input_y)
    x = ResizeFeatures((28, 28))(x)
...
def vggd_dct_deconv_resize(classes=1000, input_shape=(28, 28)):
    """Instantiates the VGG16 architecture.
        classes: optional number of classes to classify images
            into, only to be specified if `include_top` is True, and
            if no `weights` argument is specified.
...
def vggd_dct_y(classes=1000, input_shape=(28, 28)):
    """Instantiates the VGG16 architecture.
        classes: optional number of classes to classify images
            into, only to be specified if `include_top` is True, and
            if no `weights` argument is specified.

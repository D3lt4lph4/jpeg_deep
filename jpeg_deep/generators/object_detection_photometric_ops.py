...
class Hue:
    ...
    def __init__(self, delta):
        ...
        if not (-180 <= delta <= 180):
            raise ValueError(
                "`delta` must be in the closed interval `[-180, 180]`.")
        self.delta = delta
...
class RandomHue:
    ...
    def __init__(self, max_delta=18, prob=0.5):
        ...
        if not (0 <= max_delta <= 180):
            raise ValueError(
                "`max_delta` must be in the closed interval `[0, 180]`.")
    ...
    def __call__(self, image, labels=None):
        ...
        if p >= (1.0-self.prob):
            self.change_hue.delta = np.random.uniform(
...
class RandomSaturation:
    ...
    def __call__(self, image, labels=None):
        p = np.random.uniform(0, 1)
        if p >= (1.0-self.prob):
            self.change_saturation.factor = np.random.uniform(
...
class RandomBrightness:
    ...
    def __call__(self, image, labels=None):
        p = np.random.uniform(0, 1)
        if p >= (1.0-self.prob):
            self.change_brightness.delta = np.random.uniform(
                self.lower, self.upper)
...
class RandomContrast:
    ...
    def __call__(self, image, labels=None):
        p = np.random.uniform(0, 1)
        if p >= (1.0-self.prob):
            self.change_contrast.factor = np.random.uniform(
                self.lower, self.upper)
...
class Gamma:
    ...
    def __init__(self, gamma):
        ...
        self.table = np.array(
            [((i / 255.0) ** self.gamma_inv) * 255 for i in np.arange(0, 256)]).astype("uint8")
